...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!args.length) {
            ...
            const lines = categories.map((category, name) => `**${client.emoji.help[name.toLowerCase()]}  ${name} [${category.size}]**: \n\`${category.map(command => command.name).join('`, `')}\``)
            let noArgEmbed = new Discord.MessageEmbed()
                .setThumbnail(message.guild.iconURL({
                    dynamic: true
                }))
                .setDescription(`❯ To get help on a command, use \`help <command>\`\n\n\`\`\`\n[] - Optional\n<> - Required\n\`\`\`\n${lines.join('\n')}`)
