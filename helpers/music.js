  ...
  async Player(message, Discord, client, Ytdl, options = {}) {
    ...
    if (!options.Play) {
      ...
      const Embeded = new Discord.MessageEmbed()
        ...
        .setDescription(
          "There are no more songs in the queue, thanks for listening to me!"
        )
    ...
    if (Seek) {
      ...
      await Db.TextChannel.send(PlayEmbed)
      Db.ExtraTime = 0;
    };
    ...
    await Db.Bot.dispatcher
      ...
      .on("error", async error => {
        ...
        const errorEmbed = new Discord.MessageEmbed()
          ...
          .setDescription(`\`\`\`Client internal error\`\`\``)
          .setColor(client.colors.red)
          .setFooter(client.user.username, client.user.displayAvatarURL())
          .setAuthor(message.author.tag, message.author.displayAvatarURL())
        return Db.TextChannel.send(errorEmbed)
      });
  },
