...
module.exports = async (client, message) => {
  ...
  client.userProfile.ensure(message.author.id, {
    balance: 0
  })
  ...
  if (guildCmd.some(cmd => cmd.guild === message.guild.id && cmd.command === command.name)) return message.channel.send(`${client.emoji.bot.disabled} **This command is disabled globally**`)

	let globalCmd = client.disabled.get("commands", "global")
	if(globalCmd.some(cmd => cmd.command === command.name)) return message.channel.send(`${client.emoji.bot.disabled} **This command is disabled for this guild**`)
